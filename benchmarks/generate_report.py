...
def write_csv(path: Path, rows: List[Row]) -> None:
    with path.open("w", encoding="utf-8") as f:
        ...
        for r in rows:
            f.write(f"{r.workload},{r.variant},{r.name},{r.min_s:.6f},{r.median_s:.6f},{r.max_s:.6f}\n")
...
def write_bar_svg(path: Path, title: str, labels: List[str], values: List[float]) -> None:
    ...
    colors = ["#6c8cff", "#46b3a9", "#f28f3b", "#b27cff", "#ef476f", "#ffd166", "#4cc9f0"]
    ...
    parts.append(f'<text x="{margin_left}" y="36" fill="#e5e7eb" font-size="26" font-family="Helvetica, Arial">{title}</text>')
    parts.append(
    ...
    for tv in tick_vals:
        ...
        if margin_top <= y <= margin_top + plot_h:
            parts.append(f'<line x1="{margin_left}" y1="{y:.2f}" x2="{margin_left + plot_w}" y2="{y:.2f}" stroke="#334155" stroke-width="1"/>')
            parts.append(
    ...
    for i, (label, val) in enumerate(zip(labels, values)):
        ...
        parts.append(f'<rect x="{x:.2f}" y="{y:.2f}" width="{slot_w:.2f}" height="{h:.2f}" fill="{color}" opacity="0.92"/>')
        parts.append(
...
def build_markdown(rows: List[Row], plots_rel: Dict[str, str], runs: int, warmup: int) -> str:
...
def main() -> None:
    ...
    for workload, script in BENCH_SCRIPTS:
        for backend, fastpath, variant in BENCH_CONFIGS:
            cmd = ["./venv_3.11/bin/python", script, "--runs", str(args.runs), "--backend", backend]
