...
def main():
    parser = argparse.ArgumentParser(
        description="Benchmark Funk vs Python vs C using fib(35)."
    )
    parser.add_argument(
        "--runs", type=int, default=5, help="Number of timing runs per implementation."
    )
    parser.add_argument(
        "--warmup",
        type=int,
    ...
    funk_src = os.path.join(root, "benchmarks", "bench", "fib_compare.f")
    py_src = os.path.join(root, "benchmarks", "bench", "fib_compare.py")
    c_src = os.path.join(root, "benchmarks", "bench", "fib_compare.c")
